...
odin_scores_report <- read_csv("Input/ODIN Final Data_0720_BFA.csv") %>%
...
# This may change once four additional countries are added to dataset
odin_always <- odin_scores_report %>%
...
# Create consistent set of countries since 2020. 185 as of 2022 data
# This may change once four additional countries are added to dataset
odin_20_22 <- odin_scores_report %>%
...
# Number of countries with decreases in overall, coverage, or openness scores
odin_scores_report %>% 
  filter(element %in% c("Overall score", "Coverage subscore", "Openness subscore"), 
         year %in% c(2020, 2022), 
         data_categories == "All Categories") %>% 
...
# Figure 1 metadata
date_last_updated <- "20 July 2023"
...
# Figure 7 metadata
definition <- "The scores in this figure represent the median coverage, and openness scores for all data categories for a consistent set of 185 countries assessed in both ODIN 2020 and 2022 rounds."
...
# Go back to codeblock "Number of countries with decreases in overall, coverage, or openness scores" for calculation of openness and coverage scores dropping (text ahead of figure 7)
...
# Figure 8 metadata
definition <- "The scores in this figure represent the median coverage or openness element scores for all data categories for a consistent set of 185 countries assessed in both ODIN 2020 and 2022 rounds."
...
# Figure 9 metadata
definition <- "The scores in this figure represent the median overall, coverage, or openness subscores for each ODIN data category for a consistent set of 185 countries assessed in both ODIN 2020 and 2022 rounds."
...
element_change_sector("International trade", "Coverage")
...
aggregate_stats <- odin_scores_report %>%
...
inc_group_with_median <- odin_scores_report %>%
  filter(year == 2022, data_categories == "All Categories", element == "Overall score", !is.na(income_group)) %>%
  select(country, country_code, income_group, region, score) %>%
  bind_cols(odin_scores_report %>%
              filter(year == 2022, data_categories == "All Categories", element == "Overall score", !is.na(income_group)) %>%
              group_by(income_group) %>%
              summarize(median_score = median(score, na.rm = TRUE)) %>%
              ungroup() %>%
...
inc_group_with_median %>%
  filter(str_detect(income_group, "middle income"), score > `median_High income`)
inc_group_with_median %>%
  filter(income_group=="High income", score < `median_Low income`)

#### Regional Analysis
# Figure 14 Regional median scores 2020 vs 2022
region_change <- odin_scores_report %>%
...
# Figure 14 metadata
definition <- "The scores in this figure represent the change in median overall scores for all data categories by UN region for a consistent set of 185 countries assessed in both the 2020 and 2022 ODIN round."
...
# Figure 15 metadata
definition <- "The scores in this figure represent the change in median coverage or openness subscores for all data categories by UN region for a consistent set of 185 countries assessed in both the 2020 and 2022 ODIN round."
...
tibble(definition, date_last_updated, responsible_agency) %>%
  write.xlsx("Output/ODIN 2022 Table 1.xlsx", showNA = FALSE, sheetName = "ODIN 22|23 Report Tab1 metadata", append = TRUE, row.names = FALSE)
...
# Chile the most improved country in South America
odin_scores_report %>%
  filter(year %in% c(2020, 2022), element == "Overall score", data_categories == "All Categories") %>%
  semi_join(odin_20_22) %>%
  pivot_wider(id_cols = c(country, country_code, region, macro_region), names_from = year, values_from = score) %>%
...
# Chile increase in overall subscores scores
odin_scores_report %>%
  filter(country == "Chile", year %in% c(2020, 2022), str_detect(element, "score"), data_categories == "All Categories") %>%
...
# Paraguay increase in overall subscores scores
odin_scores_report %>%
  filter(country == "Paraguay", year %in% c(2020, 2022), str_detect(element, "score"), data_categories == "All Categories") %>%
  pivot_wider(id_cols = c(element, data_categories), names_from = year, values_from = score) %>%
  mutate(change = `2022` - `2020`)
...
# Australia overall score
odin_scores_report %>%
  filter(country == "Australia", year %in% c(2020, 2022), data_categories == "All Categories", str_detect(element, "score")) %>%
  pivot_wider(id_cols = element, names_from = year, values_from = score) %>%
  mutate(score_change = `2022` - `2020`)
...
# Which categories improved on openness element scores
odin_scores_report %>%
  filter(country == "Australia", year %in% c(2020, 2022), macro_element == "Openness elements", !str_detect(data_categories, "score|All")) %>% 
  pivot_wider(id_cols = c(data_categories, element), names_from = year, values_from = score) %>% 
  mutate(score_change = `2022` - `2020`) %>% 
  arrange(desc(score_change))

# Jordan rank change
odin_scores_report %>%
  filter(region == "Western Asia", year %in% c(2020, 2022), element == "Overall score", data_categories == "All Categories") %>%
  pivot_wider(id_cols = country, names_from = year, values_from = score) %>%
  mutate(score_change = `2022` - `2020`) %>%
  arrange(desc(score_change))
...
# Which data categories - elements changed the most other than Terms of use
odin_scores_report %>%
  filter(country == "Jordan", year %in% c(2020, 2022), macro_element == "Openness elements", !str_detect(data_categories, "score|All")) %>% 
  pivot_wider(id_cols = c(data_categories, element), names_from = year, values_from = score) %>% 
  mutate(score_change = `2022` - `2020`) %>% 
  arrange(desc(score_change)) %>%
  filter(element != "Terms of use")
...
tibble(definition, date_last_updated, responsible_agency) %>%
  write.xlsx("Output/ODIN 2022 Figure 17.xlsx", showNA = FALSE, sheetName = "ODIN 22|23 Report Fig17 metadata", append = TRUE, row.names = FALSE)
...
# Figure 18 metadata
definition <- "The scores in this figure represent 2022 scores for all 195 countries assessed in the ODIN 2022/23 round by their score for gender-relevant versus not gender-relevant data categories."
...
# Figure 19 metadata
definition <- "The scores in this figure represent the median overall, coverage, and openness scores for gender- or non-gender-relevant data categories for all 195 countries assessed in the ODIN 2022/23 round."
...
# Figure 20 metadata
definition <- "The scores in this figure represent the median coverage and openness element scores by gender- or non-gender-relevant data categories for all 195 countries assessed in the ODIN 2022/23 round."
