...
# Report comes from ODIN_score_2022_fix.csv file
odin_scores_report <- read_csv("Input/ODIN Final Data(missing 4 countries)_06282023.csv") %>%
...
odin_scores %>%
  ...
  summarize(med_score = median(score, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(aes(x = as.factor(year), y = med_score, color = data_categories, group = data_categories, label = round(med_score, 1))) +
  geom_line() +
  geom_text(vjust = 0, nudge_y = 0.5, show.legend = FALSE) +
  labs(x = "", y = "Median Overall score", color = "") +
  scale_x_discrete(limits = c("2016", "2017", "2018", "", "2020", "", "2022"))
ggsave("Graphs/Median macro sector scores over time.png", dpi = 400)
...
ogdi_2022_alt <- odin_scores_report %>%
...
# Compute Coverage and Openness scores for OGDI per country
(ogdi_country_alt <- ogdi_2022_alt %>%
...
(ogdi_country_historical_alt <- ogdi_2022_alt %>%
...
(ogdi_totals_alt <- ogdi_country_alt %>%
...
(ogdi_totals_historical_alt <- ogdi_country_historical_alt %>%
    semi_join(odin_always) %>%
    group_by(year) %>%
    summarize(across(starts_with("total"), list(mean = mean, median = median))) %>%
    ungroup() %>%
...
(non_ogdi_country_alt <- ogdi_2022_alt %>%
...
(non_ogdi_country_historical_alt <- ogdi_2022_alt %>%
...
(non_ogdi_totals_alt <-  non_ogdi_country_alt %>%
  # Total across all countries
  group_by(element) %>%
  summarize(total_non_gender_median = median(non_gender_score, na.rm = TRUE),
            total_non_gender_mean = mean(non_gender_score, na.rm = TRUE)) %>%
...
(non_ogdi_totals_historical_alt <-  non_ogdi_country_historical_alt %>%
    semi_join(odin_always) %>%
    # Total across all countries
    group_by(element, year) %>%
    summarize(total_non_gender_median = median(non_gender_score, na.rm = TRUE),
...
ogdi_totals_historical_alt %>%
  ...
  ggplot(aes(x = as.factor(year), y = total_median, fill = ogdi, label = round(total_median, 1))) +
  geom_col(position = position_dodge()) +
...
# Coverage scores OGDI vs non-OGDI from 2017 - 2022 for IDA-eligible countries
ogdi_country_historical_alt %>%
  select(country, country_code, year, ogdi_coverage_subscore = total_coverage_subscore) %>%
  full_join(non_ogdi_country_historical_alt %>% 
              filter(element == "Coverage subscore") %>%
              select(country, country_code, year, non_ogdi_coverage_subscore = non_gender_score)) %>%
  semi_join(odin_always) %>%
  left_join(read_csv("C:/Users/loren/Documents/R work/Input Data/wb_countries_fy24.csv") %>% 
              janitor::clean_names() %>% 
              filter(!is.na(region)) %>%
              select(code, lending_category), by = c("country_code" = "code")) %>%
  filter(lending_category %in% c("IDA", "Blend")) %>%
  group_by(year) %>%
  summarize(median_ogdi_cov = median(ogdi_coverage_subscore, na.rm = TRUE),
            median_non_ogdi_cov = median(non_ogdi_coverage_subscore, na.rm =TRUE)) %>%
  ungroup()
